  ...
  def propose(self):
    while len(self.requests) != 0 and self.slot_in < self.slot_out+WINDOW:
      if self.slot_in not in self.decisions:
        command = self.requests.pop(0)
        self.proposals[self.slot_in] = command
        for ldr in self.config.leaders:
          self.sendMessage(ldr, ProposeMessage(self.me, self.slot_in, command))
      self.slot_in +=1
      if self.slot_in > WINDOW and self.slot_in-WINDOW in self.decisions and \
